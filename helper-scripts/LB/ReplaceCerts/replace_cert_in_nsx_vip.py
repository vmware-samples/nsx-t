...
'''
Script to replace cert in existing LB-VIP
...
'''
# python replace_cert_in_nsx_vip.py -nsx_manager 192.168.110.201 -nsx_user admin -nsx_password VMware1!VMware1! -lb_vip_id vip2 -old_nsx_cert_path /infra/certificates/www1 -new_nsx_cert_name newcert -new_cert_file newcert.crt -new_key_file newcert.key
...
parser.add_argument("-lb_vip_id", help="REQUIRED - NSX LB VIP ID (usually LB VIP name)", type=str, required=True)
...
parser.add_argument("-new_nsx_cert_name", help="REQUIRED - NSX LB VIP new cert name (e.g. newcert)", type=str, required=True)
parser.add_argument("-new_cert_file", help="REQUIRED - NSX LB VIP new cert file(e.g. newcert.crt)", type=str, required=True)
parser.add_argument("-new_key_file", help="REQUIRED - NSX LB VIP new key file (e.g. newcert.key)", type=str, required=True)
args = parser.parse_args()
nsx_manager = args.nsx_manager
nsx_user = args.nsx_user
nsx_password = args.nsx_password
lb_vip_id = args.lb_vip_id
...
def nsx_replace_cert_in_vip (mgr, uri, old_nsx_cert_path, nsx_new_cert_path):
    nsx_vip_config = nsx_get_call(mgr, uri)
    result = 'no_change_done'
    for nsx_vip_config_field in nsx_vip_config:
        value = nsx_vip_config[nsx_vip_config_field]
        if type(value) is str:
            if value == old_nsx_cert_path:
                nsx_vip_config[nsx_vip_config_field] = nsx_new_cert_path
                result = 'change_done'
                print ('  - changed cert in '   +str(nsx_vip_config_field))
                ...
                if type(value_dict) is list:
                    ...
                    for entry in value_dict:
                        if str(entry) == old_nsx_cert_path:
                            nsx_vip_config[nsx_vip_config_field][key_dict][i] = nsx_new_cert_path
                            result = 'change_done'
                            print ('  - changed cert in '   +str(nsx_vip_config_field) + ' - ' + key_dict)
                        i = i+1
                elif str(value_dict) == old_nsx_cert_path:
                    nsx_vip_config[nsx_vip_config_field][key_dict] = nsx_new_cert_path
                    result = 'change_done'
                    print ('  - changed cert in '   +str(nsx_vip_config_field))
    if result == 'change_done':
        result = nsx_patch_call(mgr, uri, nsx_vip_config)
    else:
        print ('  - Old cert not present in VIP')
    return result
...
#Validate inputs
if nsx_get_call(nsx_manager, '/policy/api/v1/infra/lb-virtual-servers/'+lb_vip_id) == 'error':
    print ('  lb_vip_id '+ lb_vip_id +' does not exist')
...
#replace cert in vip
print ('2. Replace in VIP '+ lb_vip_id + ' the old cert ('+ old_nsx_cert_path +') with new cert ('+ nsx_new_cert_path +')')
result = nsx_replace_cert_in_vip(nsx_manager, '/policy/api/v1/infra/lb-virtual-servers/'+lb_vip_id, old_nsx_cert_path, nsx_new_cert_path)
